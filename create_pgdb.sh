...
DB_NAME=${1:-"pyramid"}        # Имя базы данных
DB_USER=${2:-"pyramid"}        # Имя создаваемого пользователя
DB_PASS=${3:-"1234"}           # Пароль пользователя
...
sudo -u $DB_SUPERUSER -i psql -c '\q'|| {
...
sudo -u $DB_SUPERUSER -i psql -v ON_ERROR_STOP=1 <<SQL
DO \$\$
BEGIN
...
DB_EXISTS=$(sudo -u $DB_SUPERUSER -i psql -t -c "SELECT 1 FROM pg_database WHERE datname = '${DB_NAME}'")
...
sudo -u $DB_SUPERUSER -i psql -d postgres -v ON_ERROR_STOP=1 <<SQL
-- Права на подключение к БД (выполняется в postgres)
REVOKE CONNECT ON DATABASE ${DB_NAME} FROM PUBLIC;
GRANT CONNECT ON DATABASE ${DB_NAME} TO ${DB_USER};
SQL

sudo -u $DB_SUPERUSER -i psql -d "${DB_NAME}" -v ON_ERROR_STOP=1 <<SQL
